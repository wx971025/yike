"""桌面版 API（macOS）：免登录 bootstrap + 词典按需下载 + 数据同步/更新检查。

与 Windows 版的差异：
- 更新安装包为 .dmg，校验去掉 PE(MZ) 头判断，仅校验大小 / Content-Length；
- 实际安装由 launcher.run_update_installer 用 `open <dmg>` 在 Finder 挂载，用户手动拖入 Applications（非自动安装）。
"""
...
UPDATE_INSTALLER_NAME = "YiKe-mac-arm64.dmg"
# macOS 暂不走自建镜像，直连 GitHub Release
DEFAULT_UPDATE_MIRROR_BASE = ""
...
def _parse_mirror_release() -> dict | None:
    ...
    try:
        ...
        rel_url = str(meta.get("download_url") or f"/releases/desktop-mac/{filename}").strip()
...
def _download_update_worker(target: dict) -> None:
    ...
    destination = _updates_dir() / f"YiKe-mac-{version}.dmg"
...
def desktop_update_install(request: Request):
    """返回 .dmg 路径；实际挂载由 launcher.run_update_installer 用 `open` 打开。"""
    _require_desktop(request)
    with _update_lock:
